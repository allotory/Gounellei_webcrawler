...
def login(username, password) :
    ...
    # 签到
    daily_content = s.get('https://www.v2ex.com/mission/daily').content
    daily_soup = BeautifulSoup(daily_content, 'html.parser')
    daily_once = daily_soup.find('input', attrs={'class': 'super normal button'})['onclick']
    daily_url = re.search(r'/mission/daily/redeem\?once=\d+', daily_once).group()
    
    resp = s.get('https://www.v2ex.com' + daily_url)
    print(resp.ok)
